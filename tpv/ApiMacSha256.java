...
public class ApiMacSha256 {
	 ...
	 */
	public byte[] encrypt_3DES(final String claveHex, final String datos) throws InvalidKeyException, NoSuchAlgorithmException,
		...
		// Crea la clave
		final DESedeKeySpec desKeySpec = new DESedeKeySpec(toByteArray(claveHex));
		final SecretKey desKey = new SecretKeySpec(desKeySpec.getKey(), "DESede");
		// Crea un cifrador
		final Cipher desCipher = Cipher.getInstance("DESede/CBC/NoPadding");
		...
		final byte[] cleartext = array.toByteArray();
		// Encripta el texto
	...
	public byte[] encodeB64UrlSafe(final byte[] data) {
		final byte[] encode = Base64.encodeBase64(data);
	...
	public String toHexadecimal(byte[] datos, int numBytes) {
		String resultado = "";
		final ByteArrayInputStream input = new ByteArrayInputStream(datos, 0, numBytes);
		String cadAux;
	...
	public byte[] toByteArray(String cadena) {
		...
		final int longitud = cadena.length() / 2;
		int posicion = 0;
	...
	public byte[] mac256(final String dsMerchantParameters, final byte[] secretKo)
		...
		// Se hace el MAC con la clave de la operaci�n "Ko" y se codifica en
		// BASE64
		final Mac sha256HMAC = Mac.getInstance("HmacSHA256");
		final SecretKeySpec secretKey = new SecretKeySpec(secretKo, "HmacSHA256");
		sha256HMAC.init(secretKey);
		final byte[] hash = sha256HMAC.doFinal(dsMerchantParameters.getBytes("UTF-8"));
		return hash;
	}
	...
	public String createMerchantParameters() throws UnsupportedEncodingException {
		...
		final String res = encodeB64String(jsonString.getBytes("UTF-8"));
		return res;
	}

	public String createMerchantSignature(final String claveComercio)
			throws UnsupportedEncodingException, InvalidKeyException, NoSuchAlgorithmException, IllegalStateException, NoSuchPaddingException,
			InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		final String merchantParams = createMerchantParameters();

		final byte[] clave = decodeB64(claveComercio.getBytes("UTF-8"));
		final String secretKc = toHexadecimal(clave, clave.length);
		final byte[] secretKo = encrypt_3DES(secretKc, getOrder());

		// Se hace el MAC con la clave de la operaci�n "Ko" y se codifica en
		// BASE64
		final byte[] hash = mac256(merchantParams, secretKo);
		final String res = encodeB64String(hash);
		return res;
	}

	//////////////////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////
	//////////// FUNCIONES PARA LA RECEPCI�N DE DATOS DE PAGO (Notif, URLOK y
	////////////////////////////////////////////////////////////////////////////////////////////// URLKO):
	////////////////////////////////////////////////////////////////////////////////////////////// ////////////
	//////////////////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////
	...
	public String getOrderNotifSOAP(final String datos) {
		final int posPedidoIni = datos.indexOf("<Ds_Order>");
		final int tamPedidoIni = "<Ds_Order>".length();
		final int posPedidoFin = datos.indexOf("</Ds_Order>");
		return datos.substring(posPedidoIni + tamPedidoIni, posPedidoFin);
	}

	public String getRequestNotifSOAP(final String datos) {
		final int posReqIni = datos.indexOf("<Request");
		final int posReqFin = datos.indexOf("</Request>");
		final int tamReqFin = "</Request>".length();
		return datos.substring(posReqIni, posReqFin + tamReqFin);
	}

	public String getResponseNotifSOAP(final String datos) {
		final int posResIni = datos.indexOf("<Response");
		final int posResFin = datos.indexOf("</Response>");
		final int tamResFin = "</Response>".length();
	...
	public String decodeMerchantParameters(final String datos) throws UnsupportedEncodingException {
		final byte[] res = decodeB64UrlSafe(datos.getBytes("UTF-8"));
		final String params = new String(res, "UTF-8");
	...
	public String createMerchantSignatureNotif(final String claveComercio, final String merchantParams)
		...
		final byte[] clave = decodeB64(claveComercio.getBytes("UTF-8"));
		final String secretKc = toHexadecimal(clave, clave.length);
		final byte[] secretKo = encrypt_3DES(secretKc, getOrderNotif());

		// Se hace el MAC con la clave de la operaci�n "Ko" y se codifica en
		// BASE64
		final byte[] hash = mac256(merchantParams, secretKo);
		final byte[] res = encodeB64UrlSafe(hash);
		return new String(res, "UTF-8");
	}
	...
	public String createMerchantSignatureNotifSOAPRequest(final String claveComercio, final String request)
		...
		final byte[] clave = decodeB64(claveComercio.getBytes("UTF-8"));
		final String secretKc = toHexadecimal(clave, clave.length);
		final byte[] secretKo = encrypt_3DES(secretKc, getOrderNotifSOAP(request));

		// Se hace el MAC con la clave de la operaci�n "Ko" y se codifica en
		// BASE64
		final byte[] hash = mac256(getRequestNotifSOAP(request), secretKo);
		final byte[] res = encodeB64(hash);
		return new String(res, "UTF-8");
	}
	...
	public String createMerchantSignatureNotifSOAPResponse(final String claveComercio, final String response, final String numPedido)
		...
		final byte[] clave = decodeB64(claveComercio.getBytes("UTF-8"));
		final String secretKc = toHexadecimal(clave, clave.length);
		final byte[] secretKo = encrypt_3DES(secretKc, numPedido);

		// Se hace el MAC con la clave de la operaci�n "Ko" y se codifica en
		// BASE64
		final byte[] hash = mac256(getResponseNotifSOAP(response), secretKo);
		final byte[] res = encodeB64(hash);
		return new String(res, "UTF-8");
	}
}